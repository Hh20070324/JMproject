...
v2.5.1 可以从搜索结果把漫画添加到账号的默认收藏。未登录时收藏按钮保持灰色禁用；添加成功后请前往「我的收藏」手动同步确认。当前不支持取消收藏、选择收藏文件夹或自动同步。密码不会写入磁盘；保存的会话和收藏缓存使用当前 Windows 用户的 DPAPI 加密，并且只写在程序目录。发行版已经包含 Python 与运行依赖，解压即可使用。
...
4. 登录后返回「搜索与下载」，搜索结果卡片左下角的收藏按钮会变为可用。点击后只会添加到账号的默认收藏；成功提示不会自动切换页面或开始同步。
5. 添加成功后进入「我的收藏」手动同步，确认新增漫画出现在默认收藏夹。已同步或本次运行成功添加的漫画会显示已收藏状态，不会重复提交。
6. 程序启动时只恢复本地加密缓存，不会自动连接网站。需要更新收藏时请手动点击「同步」。
7. 点击「退出登录」会在确认后删除程序目录中的 `account.dat` 和 `favorites.dat`。

当前版本只支持从搜索结果添加到默认收藏，不支持取消收藏、移动收藏、选择目标文件夹或管理远端文件夹。断网或响应不确定时不会自动重试写入，程序会提示手动同步确认最终结果。会话 Cookie 和收藏元数据整体使用 Windows DPAPI CurrentUser 加密；同一台电脑、同一 Windows 用户移动整个程序目录后仍可读取，但复制到其他电脑或其他 Windows 用户后通常无法解密。遇到“本地登录信息无法读取”时，可以清除本地账号数据并重新登录；程序不会自动删除原密文。
...
不会。v2.5.1 的登录会话只用于账号验证、读取收藏以及用户主动执行的默认收藏写入；普通搜索、封面和整本下载仍使用原有客户端链路。
