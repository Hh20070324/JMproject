...
ARCHIVE_NAME = "JM-Downloader-v3.2.2-Windows-x64.zip"
...
class PhaseEightReleaseTests(unittest.TestCase):
    def test_version_resources_match_the_release(self):
        ...
        for resource in (formal, debug):
            self.assertIn("filevers=(3, 2, 2, 0)", resource)
            self.assertIn("prodvers=(3, 2, 2, 0)", resource)
            self.assertIn("StringStruct('FileVersion', '3.2.2')", resource)
            self.assertIn("StringStruct('ProductVersion', '3.2.2')", resource)
    ...
    def test_release_name_and_checksum_are_consistent(self):
        ...
        self.assertIn('$ReleaseVersion = "3.2.2"', build_script)
        self.assertIn(
        ...
        self.assertIn("JM-Downloader-v3.2.0-Windows-x64.zip", build_script)
        self.assertIn("JM-Downloader-v3.2.1-Windows-x64.zip", build_script)
        ...
        for document in (readme, guide):
            self.assertIn("选择收藏位置", document)
            self.assertIn("管理收藏夹", document)
            self.assertIn("移回“未分类（默认位置）”", document)
            self.assertIn("手动同步", document)
            self.assertIn("不支持取消收藏或重命名收藏夹", document)
            self.assertIn("不会自动重试", document)
            self.assertIn("章节选择", document)
            self.assertIn("固定章节", document)
            self.assertIn("全部章节图片", document)
            self.assertIn("异步下载", document)
            self.assertIn("API 路线", document)
            self.assertIn("CBZ", document)
            self.assertIn("仅图片", document)
            self.assertIn("搜索历史", document)
            self.assertIn("记住密码", document)
            self.assertIn("包含已下载章节", document)
            ...
            self.assertIn("非模态", document)
            self.assertIn("150%", document)
            self.assertIn("Shift+Space", document)
            self.assertIn("连续", document)
            ...
            self.assertIn("查询与同步引擎", document)
            self.assertIn("同步线程（兼容）", document)
            self.assertIn("旧固定路线", document)
            self.assertIn("静默", document)
            self.assertIn("最多一次", document)
            self.assertIn("退出登录", document)
